...
module.exports = function (app, connection, debug, config) {
    ...
    app.get(vroot + "login", async function (req, res) {
        if (req.session.loginid !== -1) res.redirect(vroot);
        else res.render("login");
    });
    app.get(vroot + "register", async function (req, res) {
        if (req.session.loginid !== -1) res.redirect(vroot);
        else
    ...
    app.get(vroot + "useinvite", async function (req, res) {
        if (req.session.loginid !== -1) res.redirect(vroot);
        else
            if (config.login.allowregistration)
    ...
    app.post(vroot + "login", async function (req, res, next) {
        if (req.session.loginid !== -1) {
            ...
            if (sha512(req.body.password + users[0].passwordsalt).toString() === users[0].passwordhash) {
    ...
    app.post(vroot + "register", async function (req, res, next) {
        if (req.session.loginid !== -1) {
    ...
    app.post(vroot + "useinvite", async function (req, res) {
        if (req.session.loginid !== -1) {
            debug.userapi("registration only for anonymous users");
            res.redirect(vroot);
    ...
    app.post(vroot + "checkusername", async function (req, res) {
    ...
    app.post(vroot + "checkinvite", async function (req, res) {
        var resu = await connection
            .select("id")
            .from("invites")
            .where({
    ...
    app.get(vroot + "logout", async function (req, res) {
        if (req.session.loginid !== -1)
