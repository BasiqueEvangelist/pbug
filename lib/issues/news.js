...
router.get(vroot, needsPermission("issues.news"), paginate.middleware(), async(async function (req, res, next) {
    var query = (typeof req.query.q === "undefined") ? "status:open watched:yes" : req.query.q;
        ...
        q.split(" ").forEach(function (d) {
            if (d.length === 0) { return; }
                ...
                if (req.user.id === -1)
                    ...
                    if (d.slice("watched:".length) === "yes")
                        builder = builder.whereNotNull("issuewatchers.watcherid");
                    else
                        builder = builder.whereNull("issuewatchers.watcherid");
                }
            }
            else {
