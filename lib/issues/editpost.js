...
router.get(vroot + "issues/post/:post/edit", async(async function (req, res, next) {
    if (typeof req.params.post !== typeof "") res.redirect(vroot);
        ...
        if (posts.length < 1)
            res.redirect(vroot);
        else if (!((posts[0].authorid === req.user.id && await provePermission(req.user.role, "issues.editpost.own")) || await provePermission(req.user.role, "issues.editpost.other")))
            res.redirect(vroot);
        else {
