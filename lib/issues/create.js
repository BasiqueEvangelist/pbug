...
var { catchFiles, requiresLogin, insertActivity, async, needsPermission, permissionError } = require("../common");
var { provePermission } = require("../permissions");
var debug = require("../debug");

router.get(vroot + "issues/create", needsPermission("issues.create"), async(async function (req, res, next) {
...
router.post(vroot + "issues/create", needsPermission("issues.create"), catchFiles(), async(async function (req, res, next) {
    if (typeof req.body.name !== typeof "string") {
        ...
        var id = (await connection("issues")
            ...
            .returning("id"))[0];
        req.logger.log("debug", "watching issue %s for %s", id, req.user.username);
