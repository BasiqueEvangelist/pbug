...
router.get(vroot + "issues/:issue/posts", needsPermission("issues.view.posts"), async(async function (req, res, next) {
    if (typeof req.params.issue !== typeof "") {
        ...
        res.render("issues/viewtalk", {
            ...
            users: users,
            files: files,
            canopen: req.user.id === -1 ? false : (req.user.id === issues[0].assigneeid && provePermission(req.user.permissions, "issues.changestatus.own")) ||
                provePermission(req.user.permissions, "issues.changestatus.other"),
            canedit: req.user.id === -1 ? false : (issues[0].authorid === req.user.id && provePermission(req.user.permissions, "issues.edit.own")) ||
                provePermission(req.user.permissions, "issues.edit.other"),
            candelete: req.user.id === -1 ? false : provePermission(req.user.permissions, "issues.remove"),
            canpost: req.user.id === -1 ? false : provePermission(req.user.permissions, "issues.post"),
            caneditownpost: req.user.id === -1 ? false : provePermission(req.user.permissions, "issues.editpost.own"),
            caneditotherpost: req.user.id === -1 ? false : provePermission(req.user.permissions, "issues.editpost.other")
        });
    }
}));
