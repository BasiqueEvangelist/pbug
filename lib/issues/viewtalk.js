...
router.get(vroot + "issues/:issue/posts", needsPermission("issues.view.posts"), async(async function (req, res, next) {
    if (typeof req.params.issue !== typeof "") {
        ...
        res.render("issues/viewtalk", {
            ...
            canpost: req.user.id === -1 ? false : await provePermission(req.user.role, "issues.post"),
            caneditownpost: req.user.id === -1 ? false : await provePermission(req.user.role, "issues.editpost.own"),
            caneditotherpost: req.user.id === -1 ? false : await provePermission(req.user.role, "issues.editpost.other")
        });
    }
}));
