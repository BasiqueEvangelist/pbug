...
router.get(vroot + "issues/:issue/posts", async(async function (req, res, next) {
    if (typeof req.params.issue !== typeof "") {
        ...
        res.render("issues/viewtalk", {
            ...
            users: users,
            files: files,
            canopen: (req.user.id === issues[0].assigneeid && await provePermission(req.user.role, "issues.changestatus.own." + issues[0].shortprojectid)) ||
                await provePermission(req.user.role, "issues.changestatus.other." + issues[0].shortprojectid)
        });
    }
}));
