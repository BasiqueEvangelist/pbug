...
router.get(vroot + "issues/:issue/edit", requiresLogin, async(async function (req, res, next) {
    if (typeof req.params.issue !== typeof "") res.redirect(vroot);
        ...
        if (issues.length < 1)
            res.redirect(vroot);
        else if (!((issues[0].authorid === req.user.id && await provePermission(req.user.role, "issues.edit.own")) || await provePermission(req.user.role, "issues.edit.other")))
