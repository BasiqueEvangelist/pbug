...
router.get(vroot + "issues/:issue/activity", async(async function (req, res, next) {
    ...
    res.render("issues/viewactivity", {
        ...
        things: pactivities,
        users: users,
        canopen: (req.user.id === issues[0].assigneeid && await provePermission(req.user.role, "issues.changestatus.own")) ||
