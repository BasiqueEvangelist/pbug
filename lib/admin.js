var { requiresAdministrator, async } = require("./common");
var paginate = require("express-paginate");
module.exports = function (app, connection, debug, config) {
    var vroot = config["virtual-root"];

    app.get(vroot + "admin/", requiresAdministrator, async(async function (req, res, next) {
        if (!req.user.isadmin) res.redirect(vroot);
        else res.render("admin/index");
    }));
    app.get(vroot + "admin/createproject", requiresAdministrator, async(async function (req, res, next) {
        if (!req.user.isadmin) res.redirect(vroot);
        else res.render("admin/createproject");
    }));
    app.post(vroot + "admin/createproject", requiresAdministrator, async(async function (req, res, next) {
    ...
    app.get(vroot + "admin/users", requiresAdministrator, async(async function (req, res, next) {
    ...
    app.get(vroot + "admin/invites", requiresAdministrator, async(async function (req, res, next) {
    ...
    app.post(vroot + "admin/invites/create", requiresAdministrator, async(async function (req, res, next) {
        var uid = require("crypto").randomBytes(48).toString("base64").replace(/\//g, "_").replace(/\+/g, "-");
        await connection("invites")
            .insert({
                "uid": uid,
                "isadministrator": typeof req.body.isadmin !== typeof undefined
            });
        res.redirect(vroot + "admin/invites");
    }));
    app.get(vroot + "admin/invites/:invite/remove", requiresAdministrator, async(async function (req, res, next) {
