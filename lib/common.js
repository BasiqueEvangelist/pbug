...
exports.needsPermission = function (perm) {
    return [exports.requiresLogin, exports.async(async function (req, res, next) {
        if (!await provePermission(req.user.id, perm))
            exports.permissionError(req, res, perm);
        else
            next();
    })];
};

exports.permissionError = function (req, res, permission) {
    if (req.method === "GET" || req.method === "HEAD")
        res.status(403).render("errors/403permission", { permission: permission });
    else
        res.status(403).json({
            error: {
                type: "NotAuthorized",
                message: "This service requires permission " + permission + "."
            },
            server: "PBug/0.1"
        }).end();
};
