...
router.get(vroot + "kb", async function (req, res) {
    res.redirect(vroot + "kb/search");
});
router.get(vroot + "kb/search", paginate.middleware(), async function (req, res, next) {
    ...
    function buildfrom(q, order) {
        ...
        q.split(" ").forEach(function (d) {
            if (d.length == 0) { return; }
                ...
                builder = builder.where("infopages.pagename", "ilike", "%" + d + "%");
            }
        });
        if (order)
